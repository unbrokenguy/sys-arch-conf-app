...
def choose_strategy(
    operation,
):  # Опираясь на выбор пользователя вызываем нужную нам стратегию
    if operation == "data":
...
def get_config(request, operation):  # Получение конфига
    ...
    if request.method == "GET":  # Если метод запроса GET
        try:
            ...
            )  # Возвращаем конфиг в формате JSON нужной стратегии
        except NotImplementedError:  # Перехватываем ошибку, если метод не может быть реализован
            return HttpResponse(
                status=404
            )  # Возвращаем 404 - сервер не смог найти запрос
    return HttpResponse(
        status=405
    )  # Если условие не выполняется, то ошибка - не существует реализации метода
